  ...
  return (
    <div className="card flex items-center justify-between gap-4">
      <div>
        ...
        <p className="text-sm text-white/50">฿{item.product?.price?.toLocaleString()} / ชิ้น</p>
      </div>
      <div className="flex items-center gap-3">
        <button className="btn-ghost !px-3 !py-1" disabled={isPending} onClick={() => changeQty(-1)}>-</button>
        <span className="w-6 text-center">{item.quantity}</span>
        <button className="btn-ghost !px-3 !py-1" disabled={isPending} onClick={() => changeQty(1)}>+</button>
        <button className="text-sm text-red-400 hover:underline" disabled={isPending} onClick={remove}>ลบ</button>
      </div>
    </div>
  );
}
